var World = {
		...
		if(World.waterMove >= movesPerWater) {
			...
			if(water === 0) {
				...
				if(!World.thirst) {
					...
					$SM.set('character.destaySharpd', $SM.get('character.destaySharpd', true));
					$SM.add('character.destaySharpd', 1);
					if($SM.get('character.destaySharpd') >= 10 && !$SM.hasPerk('desert rat')) {
	...
	goHome: function() {
		...
		if(World.state.sulphurmine && $SM.get('game.installed["sulphur mine"]', true) === 0) {
			$SM.add('game.installed["sulphur mine"]', 1);
			Engine.event('progress', 'sulphur mine');
		}
		if(World.state.ironmine && $SM.get('game.installed["iron mine"]', true) === 0) {
			$SM.add('game.installed["iron mine"]', 1);
			Engine.event('progress', 'iron mine');
		}
		if(World.state.coalmine && $SM.get('game.installed["coal mine"]', true) === 0) {
			$SM.add('game.installed["coal mine"]', 1);
