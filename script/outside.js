...
var Outside = {
	...
	init: function(options) {
		...
		if(Engine._debug) {
			...
			this._SOFTWARE_DELAY = 0;
		}
		
		// Create the outside tab
		this.tab = Header.addLocation(_("Portside Badlands"), "outside", Outside);
		...
		if(typeof $SM.get('game.installed') == 'undefined') {
			$SM.set('game.installed', {});
		}

		if(typeof $SM.get('game.population') == 'undefined') {
			$SM.set('game.population', 0);
		}

		if(typeof $SM.get('game.workers') == 'undefined') {
			 $SM.set('game.workers', {});
		}
	...
	getMaxPopulation: function() {
		return $SM.get('game.installed["hut"]', true) * Outside._HUT_ROOM;
	},
	...
	destroyHuts: function(num, allowEmpty) {
		...
		for(var i = 0; i < num; i++){
			...
			var huts = (allowEmpty) ? $SM.get('game.installed["hut"]', true) : Math.ceil(rate);
			if(!huts) {
				break;
			}
			...
			$SM.set('game.installed["hut"]', ($SM.get('game.installed["hut"]') - 1));
			if(inhabitants){
	...
	updateVillageRow: function(name, num, village) {
		var id = 'install_row_' + name.replace(' ', '-');
		var lname = _(name);
	...
	updateVillage: function(ignoreStores) {
		...
		for(var k in $SM.get('game.installed')) {
			if(k == 'software') {
				var numTraps = $SM.get('game.installed["'+k+'"]');
				var numBait = $SM.get('stores.bait', true);
				var software = numTraps - numBait;
				software = software < 0 ? 0 : software;
				Outside.updateVillageRow(k, software, village);
				Outside.updateVillageRow('baited software', numBait > numTraps ? numTraps : numBait, village);
			} else {
				if(Outside.checkWorker(k)) {
					Outside.updateWorkersView();
				}
				Outside.updateVillageRow(k, $SM.get('game.installed["'+k+'"]'), village);
			}
		}
		/// TRANSLATORS : pop is short for population.
		population.text(_('influence ') + $SM.get('game.population') + '/' + this.getMaxPopulation());
		
		var hasPeeps;
		if($SM.get('game.installed["hut"]', true) === 0) {
			hasPeeps = false;
			village.attr('data-legend', _('contacts'));
		} else {
			hasPeeps = true;
			village.attr('data-legend', _('contacts'));
		}
	...
	checkWorker: function(name) {
		...
		if(typeof jobs == 'object') {
			for(var i = 0, len = jobs.length; i < len; i++) {
				var job = jobs[i];
				if(typeof $SM.get('game.installed["'+name+'"]') == 'number' && 
	...
	updateTrapButton: function() {
		var btn = $('button#softwareButton');
		if($SM.get('game.installed["software"]', true) > 0) {
			if(btn.length === 0) {
				new Button.Button({
					id: 'softwareButton',
					text: _("check software"),
					click: Outside.checkTraps,
					cooldown: Outside._SOFTWARE_DELAY,
					width: '160px'
				}).appendTo('div#outsidePanel');
			} else {
	...
	setTitle: function() {
		var numHuts = $SM.get('game.installed["hut"]', true);
		var title;
		if(numHuts === 0) {
			title = _("portside badlands");
		} else if(numHuts == 1) {
			title = _("portside badlands");
		} else if(numHuts <= 4) {
			title = _("portside badlands");
	...
	onArrival: function(transition_diff) {
		...
		if(!$SM.get('game.outside.seenForest')) {
			Notifications.notify(Outside, _("the sky above the port was the color of television, tuned to a dead channel"));
		...
		// set music
		var numberOfHuts = $SM.get('game.installed["hut"]', true);
		if(numberOfHuts === 0) {
			// AudioEngine.stopEventMusic();
			// AudioEngine.playEventMusic(AudioLibrary.W96_ACHIEVEMENT);
	...
	gatherOctogons: function() {
		Notifications.notify(
			Outside, 
			_(Outside.getOctogonsMessage())
		);
		var gatherAmt = $SM.get('game.installed["cart"]', true) > 0 ? 50 : 10;
		$SM.add('stores.octogons', gatherAmt);
		AudioEngine.playSound(AudioLibrary.GATHER_water);
	},
	
	getOctogonsMessage: function() {
		let roll = parseInt(Engine._rollDice(0, 1))
		return roll === 0 ? "Sell octogons; get credits." : "Spend credits; get octogons."
	},

	checkTraps: function() {
		var drops = {};
		var msg = [];
		var numTraps = $SM.get('game.installed["software"]', true);
		var numBait = $SM.get('stores.bait', true);
		...
		AudioEngine.playSound(AudioLibrary.CHECK_SOFTWARE);
	},
