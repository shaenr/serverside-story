...
Events.Outside = [
	...
	title: _('A Ruined Trap'),
		isAvailable: function() {
			return Engine.activeModule == Outside && $SM.get('game.installed["software"]', true) > 0;
		},
		scenes: {
			'start': {
				text: [
					_('some of the software have been torn apart.'),
					_('large prints lead away, into the forest.')
				],
				onLoad: function() {
					var numWrecked = Math.floor(Math.random() * $SM.get('game.installed["software"]', true)) + 1;
					$SM.add('game.installed["software"]', -numWrecked);
					Outside.updateVillage();
					Outside.updateTrapButton();
				},
				notification: _('some software have been destroyed'),
				blink: true,
				buttons: {
					'track': {
						text: _('track them'),
		...
		audio: AudioLibrary.EVENT_RUINED_SOFTWARE
	},
	{ /* Hut fire */
		title: _('Fire'),
		isAvailable: function() {
			return Engine.activeModule == Outside && $SM.get('game.installed["hut"]', true) > 0 && $SM.get('game.population', true) > 50;
		},
		scenes: {
			'start': {
				text: [
