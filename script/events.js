...
var Events = {
	...
	options: { 'width': 1000 }, // Nothing for now
	delayState: 'wait',
	activeScene: null,
	...
	// If no events exist in the event stack, determine whaat events are possible, and schedule events to occur.
	triggerEvent: function(explicitEventStart) {
		if (!typeof explicitEventStart === 'undefined') {
	...
	// Selects the event at the top of the event stack or nothing.
	...
	startEvent: function(event, options) {
		if(event) {
			...
			let newBG = typeof Events.activeEvent().image !== 'undefined' ? null : Events.activeEvent().image;
			if (newBG) {
				document.querySelectorAll('.eventPanel')[0].style.setProperty("--event-bg", `url('${newBG}')`);
			}
		}
	},
