...
var StateManager = {
	...
	init: function(options) {
		...
		//create categories
		var cats = [
			'features',     // big features like installed, location availability, unlocks, etc
			'stores',       // little stuff, items, weapons, etc
			'character',    // this is for player's character stats such as perks
			'device',       // info about installed softwarae and device specs, services
			'income',
			'timers',
	...
	fireUpdate: function(stateName, save){
		...
		if(stateName === undefined) stateName = category = 'all'; //best if this doesn't happen as it will 
		// trigger more stuff
	...
	updateOldState: function(){
		...
		if(version == 1.2) {
			...
			if($SM.get('room')){
				...
				$SM.set('game.fixer.level', $SM.get('room.fixer'));
				$SM.remove('room');
			}
			if($SM.get('outside')){
				$SM.set('features.location.outside', true);
				$SM.set('game.population', $SM.get('outside.population'));
				$SM.set('game.installed', $SM.get('outside.installed'));
			...
			if($SM.get('world')){
				...
				$SM.set('destaySharpd', $SM.get('character.destaySharpd', true));
				$SM.remove('world');
				$SM.remove('starved');
				$SM.remove('destaySharpd');
			}
			if($SM.get('ship')){
	...
	num: function(name, craftable) {
		...
		case 'install':
			return $SM.get('game.installed["'+name+'"]', true);
		}
	},
